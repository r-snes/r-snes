...
fn test_write_cgram_data_two_writes() {
    ...
    assert!(ppu.latch_filled);
    assert_eq!(ppu.cgram[0], 0); // not written yet

    // Second write
    ppu.write_cgram_data(0x12);
    assert!(!ppu.latch_filled);
    assert_eq!(ppu.cgram[0], 0x1234 & 0x7FFF);
}
...
fn test_write_cgram_auto_increment() {
    ...
    assert_eq!(ppu.cgaddr, 1); // auto-incremented
}
...
fn test_multiple_colors() {
    ...
    for i in 0..5 {
        ...
        assert_eq!(ppu.cgram[i as usize], color & 0x7FFF);
    }
}
...
fn test_write_and_read_last_cgram_entry() {
    ...
    assert_eq!(ppu.cgram[last_index], ((0xBBu16 << 8) | 0xAA) & 0x7FFF);
}
